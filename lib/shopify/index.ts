...
export async function getCollections(): Promise<Collection[]> {
  // Comentado temporalmente para evitar llamadas a Shopify
  return [
    {
      handle: '',
      title: 'All',
...
export async function getDiscountedProducts({
  ...
  const allProducts = reshapeProducts(removeEdgesAndNodes(res.body.data.products));

  // Filtrar productos que tengan al menos una variante con compareAtPrice
...
export async function getHomeHero(): Promise<HomeHero | null> {
  // Comentado temporalmente para evitar llamadas a metaobjects
  return null;
